        ...
        while (defenderIt != npcs.end()) {
            ...
            if (attacker->isClose(defender, distance)) {
                ...
                if (defender->isDragon()) {
                    std::cout << "Dragon vs. ";
                    success = attacker->fight(std::static_pointer_cast<Dragon>(defender));
                } else if (defender->isKnight()) {
                    std::cout << "Knight vs. ";
                    success = attacker->fight(std::static_pointer_cast<Knight>(defender));
                } else if (defender->isElf()) {
                    std::cout << "Elf vs. ";
                    success = attacker->fight(std::static_pointer_cast<Elf>(defender));
                }

                // Если бой успешен, удаляем защитника из локации
                if (success) {
                    std::cout << " (Successful)" << std::endl;
                    defenderIt = npcs.erase(defenderIt);
                } else {
